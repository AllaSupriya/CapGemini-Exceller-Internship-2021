...
public class MyGlobalExceptionHandler {
	...
	public ResponseEntity<MyExceptionResponse> invalidCostInputByUser(InvalidCostRangeException ex) {
		...
		exeResponse.setExceptionMsg("Invalid Cost Range "+range1+" and "+range2+", pls try again");
	...
	public ResponseEntity<MyExceptionResponse> invalidProductId(java.util.NoSuchElementException ex) {
		...
		MyExceptionResponse exeResponse = new MyExceptionResponse();
		exeResponse.setHttpStatus(HttpStatus.BAD_REQUEST.value());
		exeResponse.setExceptionMsg("Invalid Product Id , pls try again");
		
		return new ResponseEntity<MyExceptionResponse>(exeResponse,HttpStatus.BAD_REQUEST);
	}
	
	@ExceptionHandler
	public ResponseEntity<MyExceptionResponse> invalidCategoryException(InvalidCategoryException ex) {
		String c = ex.getCategory();		
		MyExceptionResponse exeResponse = new MyExceptionResponse();
		exeResponse.setHttpStatus(HttpStatus.BAD_REQUEST.value());
		exeResponse.setExceptionMsg("Invalid Category "+c+" doesn't exist , pls try again");
